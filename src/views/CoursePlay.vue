<template>
  <div class="main">
    ...
    <div class='play'>
      <div class='play-left'>
        <vue3VideoPlay v-bind="options" @timeupdate="onCurrentTime" @loadstart="onLoadStart" />
      </div>
        ...
        <el-tabs tab-position="right">
            ...
            <div class="course-container">
              ...
              <div class="teacher" v-if="true">
                ...
                <div class="teacherInfo">
                  <div class="teacherName">{{courseInfo.teacher}}</div>
          ...
          <el-tab-pane>
            ...
            <div class="chapter-container">
              <dl class="list">
                <dt title="item.chapterName">{{ chapterInfo.chapterName }}</dt>
                <dd class="'active'" v-for="item in chapterInfo.children" :key="item.id">
                  <div class="video-itemIcon" ref="renderMap(child)">
                    <i class="el-icon-video-camera"></i>
                  </div>
                  <div class="item-name" @click="changeVideoPlay(item.id)">
                    {{ item.chapterName }}
                  </div>
                </dd>
              </dl>
...
import { getPlayInfo } from '../utils/api/courseManage.js'
import { setPlayTime, getPlayTime } from '../utils/api/user.js'
...
// 获取路由参数
let { courseId, chapterId, subChapterId } = route.params
// 视频播放的配置
...
// 视频载入
const onLoadStart = async ev => {
  const currentTime = await getPlayTime({
    userId: userStore.userinfo.id,
    courseId,
    chapterId,
    subChapterId
  })
  ev.target.currentTime = currentTime.data.playTime
}
const getImgSrc = avt => {
  return new URL(`../assets/img/teacher/${avt}`, import.meta.url).href
}
onBeforeMount(async () => {
  const res = await getPlayInfo({ courseId, chapterId, subChapterId })
  // console.log(res)
  courseInfo.value = res.courseInfo
  chapterInfo.value = res.chapterInfo
  // 视频路径
  options.src = res.playInfo.playUrl
  // console.log(res.playInfo.playUrl)
})
const changeVideoPlay = async id => {
  subChapterId = id
  const res = await getPlayInfo({ courseId, chapterId, subChapterId: id })
  options.src = res.playInfo.playUrl
}
</script>
<style scoped>
